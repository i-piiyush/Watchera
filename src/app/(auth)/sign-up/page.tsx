...
const syncUser = async (user: User, name?: string) => {
  const token = await user.getIdToken(true);
  await axios.post<ApiResponse<AppUser>>("/api/auth/sync-user", {
    idToken: token,
    ...(name ? { name } : {}),
  });
};
...
const SignUp = () => {
  ...
  const onSubmit = async (data: z.infer<typeof signupSchema>) => {
    ...
    try {
      ...
      await syncUser(res.user, cleanName);
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      await syncUser(res.user, res.user.displayName || undefined);
  ...
  const handleAuthError = (error: unknown) => {
    if (error instanceof FirebaseError) {
      ...
      toast.error(map[error.code] ?? "Authentication failed");
      return;
    }
  ...
  return (
    <div className="flex h-[90vh] overflow-hidden px-3 py-8 w-full bg-zinc-50">
      ...
      <div className="tracking-tighter w-full md:w-[40%] px-4 flex flex-col items-center justify-center gap-7">
        ...
        <div className="w-full max-w-80 flex flex-col">
          ...
          >
            <svg width="20" height="20" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
              <path fill="#4285F4" d="M24 9.5c3.54 0 6.7 1.22 9.19 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z" />
              <path fill="#34A853" d="M46.98 24.55c0-1.64-.15-3.21-.43-4.73H24v9.02h12.95c-.56 3.02-2.26 5.58-4.78 7.3l7.73 6.01C44.43 37.78 46.98 31.72 46.98 24.55z" />
              <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24s.92 7.54 2.56 10.78l7.97-6.19z" />
              <path fill="#EA4335" d="M24 48c6.48 0 11.93-2.13 15.9-5.81l-7.73-6.01c-2.15 1.45-4.92 2.3-8.17 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z" />
            </svg>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full max-w-80">
